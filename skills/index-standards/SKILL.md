...
description: Rebuild and maintain the .ai/standards/index.yml file. Scans for new/deleted standard files and updates the index that /inject-standards uses for matching. Run after manual changes to standards files.
...
1. List all `.md` files in `.ai/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `.ai/standards/` (not in a subfolder). Do not create an actual folder named "root".

### Step 2: Load Existing Index

Read `.ai/standards/index.yml` if it exists. Note which entries already have descriptions.
...
Generate `.ai/standards/index.yml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `.ai/standards/` (not in subfolders).
...
Updates `.ai/standards/index.yml`
