...
description: Inject relevant standards from .ai/standards/ into the current context. Runs in auto-suggest mode (no args) or explicit mode (folder/file args like "api" or "api/response-format"). Use at the start of a task, before implementation.
disable-model-invocation: true
argument-hint: "[folder] [folder/file] ..."
allowed-tools: Read Glob
---

Inject relevant standards into the current context, formatted appropriately for the situation.

Arguments (if any) are provided as `$ARGUMENTS` and trigger Explicit Mode below.
...
> "Your AGENTS.md setup is incomplete. Sections still marked `_(fill in)_`: [list them]. Fill them in by hand, then re-run `/inject-standards`. I will not proceed until they are filled, and I will not fill them for you — these values must come from you because they encode project-specific knowledge I cannot infer."
...
Only if all four sections are filled in → continue to Usage Modes and Process below.
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `.ai/standards/` (not in a subfolder).
...
3. **Shaping/Planning** — In plan mode, building a spec, running `/create-spec`
...
Read `.ai/standards/index.yml` to get the list of available standards and their descriptions.
...
@.ai/standards/api/response-format.md
@.ai/standards/api/error-handling.md
@.ai/standards/global/naming.md
...
When arguments are provided (`$ARGUMENTS` is non-empty), skip the suggestion step but still detect scenario.
...
Arguments can be:
- **Folder name** — `api` → inject all `.md` files in `.ai/standards/api/`
- **Folder/file** — `api/response-format` → inject `.ai/standards/api/response-format.md`
- **Root folder** — `root` → inject all `.md` files directly in `.ai/standards/` (not in subfolders)
- **Root file** — `root/naming` → inject `.ai/standards/naming.md`
...
This skill is called internally by `/create-spec` to inject relevant standards during planning. You can also invoke it directly anytime you need standards in context.
