...
description: Extract tribal knowledge from the codebase into concise, documented standards. Use at project start or when entering a new area of the codebase — analyzes patterns, asks why, and writes standards to .ai/standards/.
...
> "Your AGENTS.md setup is incomplete. Sections still marked `_(fill in)_`: [list them]. Fill them in by hand, then re-run `/discover-standards`. I will not proceed until they are filled, and I will not fill them for you — these values must come from you because they encode project-specific knowledge I cannot infer."
...
Check if the user specified an area when invoking this skill (available as `$ARGUMENTS`). If they did, skip to Step 2.
...
1. Scan `.ai/standards/` for all `.md` files
...
3. Update `.ai/standards/index.yml`:
...
All standards: `.ai/standards/[folder]/[standard].md`
Index file: `.ai/standards/index.yml`
